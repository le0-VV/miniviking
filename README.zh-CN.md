...
`miniviking` 是一个为 OpenViking 准备的轻量本地 MLX 服务器。它在 localhost 上暴露 OpenAI 兼容接口，让原版 OpenViking 安装几乎不需要额外配置，就能使用本地聊天大模型和embedding模型。
...
安装时会检测主机内存，并把配置写入 `~/.miniviking/config.json`。端口、运行模式和模型都可以在该文件里修改，也可以在安装时通过 CLI 参数自定义。
